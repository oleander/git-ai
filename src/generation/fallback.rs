...
pub async fn generate_with_fallback(diff: &str, config: &AppConfig) -> Result<String> {
  // Build strategy list
  ...
  for strategy in strategies {
    ...
    match strategy.generate(diff, config).await {
      ...
      Err(e) => {
        ...
        if error_msg.contains("invalid_api_key") || error_msg.contains("Invalid API key") || error_msg.contains("Incorrect API key") {
          return Err(e);
        }
