...
/// Optimized version that runs analyses in parallel
pub async fn generate_commit_message_multi_step_optimized(
  client: &Client<OpenAIConfig>, model: &str, diff_content: &str, max_length: Option<usize>
) -> Result<String> {
  log::info!("Starting optimized multi-step commit message generation");
  ...
  // Step 1: Analyze each file individually in parallel using tokio tasks
  log::debug!("Spawning {} parallel file analysis tasks", parsed_files.len());
  ...
  let analysis_handles: Vec<JoinHandle<_>> = parsed_files
    ...
    .map(|(idx, file)| {
      ...
      tokio::spawn(async move {
        log::debug!("Task {}: Starting analysis for {}", idx + 1, file_path);
        ...
        let result = call_analyze_function(&client, &model, &file_clone).await;
        let duration = start_time.elapsed();
        log::debug!("Task {}: Completed analysis for {} in {:?}", idx + 1, file_path, duration);
        (file_clone, result, duration, payload)
      })
    })
    .collect();

  // Wait for all analyses to complete
  log::debug!("Waiting for all file analysis tasks to complete");
  ...
  for (i, (file, result, duration, payload)) in analysis_results.into_iter().enumerate() {
    match result {
      Ok(analysis) => {
        log::debug!("Successfully analyzed file {}: {}", i + 1, file.path);
  ...
  // Prepare data for steps 2 and 3
  ...
  let candidates = call_generate_function(client, model, scored_files.clone(), max_length.unwrap_or(72)).await?;
...
pub async fn generate_commit_message_multi_step(
  ...
  generate_commit_message_multi_step_optimized(client, model, diff_content, max_length).await
}
